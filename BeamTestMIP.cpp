...
void BeamTestMIP() {
  ...
  TH1F *chargeH[16][36];
  TH2F *hitMap = new TH2F("hitMap","HBU light yield map in DESY;y [mm];x [mm]",24,-360,360,24,-360,360);
  ...
  for (int i=0;i<16;i++) {
    for (int j=0;j<36;j++) {
      sprintf(histoName,"chargeH%d_%d",129+i,j+1);
      chargeH[i][j] = new TH1F(histoName,"light yield histogram;Npe",70,0,70);
  ...
  FILE *output = fopen("../txt/testBeamAllMIP2.txt","w");
  for (int i=0;i<16;i++) {
    for (int j=0;j<36;j++) {
      langaus(chargeH[i][j], &f1, &peakP, &peakError);
      if (i==2 || i==3) {
  	hitMap->Fill(345*(3-i)+165*(i-2)-30*(j%6),-15-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==6 || i==7) {
  	hitMap->Fill(-15*(7-i)-195*(i-6)-30*(j%6),-15-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==10 || i==11) {
  	hitMap->Fill(345*(11-i)+165*(i-10)-30*(j%6),345-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==14 || i==15) {
  	hitMap->Fill(-15*(15-i)-195*(i-14)-30*(j%6),345-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==0 || i==1) {
  	hitMap->Fill(195*(1-i)+15*i+30*(j%6),-195-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==4 || i==5) {
  	hitMap->Fill(-165*(5-i)-345*(i-4)+30*(j%6),-195-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==8 || i==9) {
  	hitMap->Fill(195*(9-i)+15*(i-8)+30*(j%6),165-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      if (i==12 || i==13) {
  	hitMap->Fill(-165*(13-i)-345*(i-12)+30*(j%6),165-30*int(j/6),peakP);
  	fprintf(output,"%d %d\t%f\n", i, j, peakP);
      }
      c1->cd();
      chargeH[i][j]->Draw();
      f1->Draw("same");
      sprintf(canvasName,"../png/24_24_600fF2/fit%d_%d.png",i,j);
